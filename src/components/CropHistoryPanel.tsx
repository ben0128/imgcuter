...
const CropHistoryPanel: React.FC<CropHistoryPanelProps> = React.memo(({ history, onClearHistory, onDeleteHistory }) => {
  return (
    <div>
      <h2>Crop History</h2>
      <button onClick={onClearHistory} style={{ marginBottom: '10px' }}>
        Clear History
      </button>
      {history.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          {history.map((item, index) => (
            <div key={index} style={{ border: '1px solid #ccc', padding: '10px', borderRadius: '5px' }}>
              ...
              <button onClick={() => navigator.clipboard.writeText(item.croppedImage)}>CopyUrl</button>
